...
import (
	...
	"github.com/coreators/voter/testutil/network"
	"github.com/coreators/voter/x/voter/client/cli"
...
func networkWithPollObjects(t *testing.T, n int) (*network.Network, []types.Poll) {
	...
	for i := 0; i < n; i++ {
		state.PollList = append(state.PollList, types.Poll{
			Id: uint64(i),
		})
	}
...
func TestListPoll(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			require.Subset(t, objs, resp.Poll)
		}
	})
